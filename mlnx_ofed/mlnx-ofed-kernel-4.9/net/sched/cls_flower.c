#ifndef CONFIG_COMPAT_KERNEL_4_14
...
static void fl_hw_destroy_filter(struct tcf_proto *tp, struct cls_fl_filter *f)
{
	...
	mlx5e_delete_flower(dev, priv, &offload, f->mlx5e_flags);
}
				...
				struct cls_fl_filter *f)
{
	...
	priv = netdev_priv(f->hw_dev);
	err = mlx5e_configure_flower(dev, priv, &offload, f->mlx5e_flags);
	if (!err)
...
static void fl_hw_update_stats(struct tcf_proto *tp, struct cls_fl_filter *f)
{
	...
	priv = netdev_priv(dev);
	mlx5e_stats_flower(dev, priv, &offload, f->mlx5e_flags);
}
#else /* CONFIG_COMPAT_KERNEL_4_14 */
/*
 * net/sched/cls_flower.c		Flower classifier
 *
 * Copyright (c) 2015 Jiri Pirko <jiri@resnulli.us>
