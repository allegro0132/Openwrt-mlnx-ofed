...
OLDEST_KERNEL_SUPPORTED="2.6.16"
...
for i in $(egrep '^CONFIG_|^export CONFIG_|^ifdef CONFIG_|^ifndef CONFIG_|^endif #CONFIG_|^else #CONFIG_' $COMPAT_CONFIG | sed 's/ /+/'); do
	case $i in
	'ifdef+CONFIG_'* )
	...
	'export+CONFIG_'* )
		...
		VAR=$(echo $i | cut -d"=" -f 1 | cut -d"+" -f 2)
